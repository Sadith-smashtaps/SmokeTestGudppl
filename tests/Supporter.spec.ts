...
test.describe.serial('API Testing', () => {
    ...
    test('Create new user with OTP and email verification for joining organization @create', async ({ request, page }) => {
        const response = await request.post(urlOPT);
        const responseBody = JSON.parse(await response.text());
        expect(response.status()).toBe(201);
        getOPTURL = responseBody.uuid;
        ...
        await page.goto('https://next.gudppl.com/signup');
        await page.getByPlaceholder('Enter your email address').fill(responseBody.uuid + '@email.webhook.site');
        email2 = responseBody.uuid + '@email.webhook.site';
        await page.getByPlaceholder('Enter a new password').fill(pwd);
        await page.getByRole('checkbox').check();
        await page.getByRole('button', { name: 'Create account' }).click();
        await page.waitForTimeout(47000);
    ...
    test('Login with new verified user, complete profile preferences, and join the created organization @reg', async ({ page }) => {
        ...
        await page.getByRole('button', { name: 'Continue', exact: true }).click();
        await page.waitForTimeout(1000);
        await page.getByPlaceholder('Enter your first name').fill('James');
        await page.getByPlaceholder('Enter your last name').fill('Anderson');
        await page.getByPlaceholder('DD').fill('15');
        await page.getByPlaceholder('MM').fill('12');
        await page.getByPlaceholder('YYYY').fill('1985');
        await page.locator('label').filter({ hasText: 'boy/man' }).getByLabel('controlled').check();
        await page.getByRole('button', { name: 'Next' }).click();
        await page.waitForTimeout(3400);
        await page.getByLabel('Animal welfare').check();
        await page.getByRole('button', { name: 'Next' }).click();
        ...
        await page.getByRole('row', { name: 'Sinhala delete' }).getByRole('checkbox').nth(3).check();
        await page.getByRole('row', { name: 'Tamil delete' }).getByRole('checkbox').nth(4).check();
        //await page.getByPlaceholder('Add skills and talents').fill('Volunteer');
        //await page.getByRole('option', { name: 'Volunteer', exact: true }).click();
        //await page.getByRole('option', { name: 'Food bank volunteering', exact: true }).click();
        //await page.getByRole('row', { name: 'English' }).getByRole('checkbox').nth(1).check();
        //await page.getByRole('row', { name: 'Sinhala' }).getByRole('checkbox').nth(2).check();
        //await page.getByRole('row', { name: 'Tamil delete' }).getByRole('checkbox').nth(3).check();
        await page.getByRole('button', { name: 'Next' }).click();
        await page.waitForTimeout(3400);
        await page.getByRole('row', { name: 'Monday' }).getByRole('checkbox').nth(1).check();
        await page.getByRole('row', { name: 'Tuesday' }).getByRole('checkbox').nth(2).check();
        ...
        await page.getByRole('button', { name: 'Next' }).click();
        await page.waitForTimeout(4000);
        await page.getByPlaceholder('Your phone number').fill('774455887');
        await page.getByPlaceholder('Select your country').fill('Sri');
        await page.getByRole('option', { name: 'Sri Lanka' }).click();
        await page.waitForTimeout(500);
        await page.locator("//label[normalize-space()='Country']/../..//div[2]//div//div//div//div[2]").click();
        await page.locator("//label[normalize-space()='Country']/../..//div[2]//div//div//div//div[2]//input").fill('Colombo');
        await page.getByText('Colombo', { exact: true }).click();
        await page.getByPlaceholder('Write few sentences about you').fill('Hi, my name is James Anderson');
        await page.getByRole('button', { name: 'Complete' }).click();
        await page.waitForTimeout(3500);
        await expect.soft(page.getByRole('heading', { name: 'Hello, James. Welcome to gudppl!' })).toHaveText("Hello, James. Welcome to gudppl!");
        ...
        expect.soft(page.locator('div').filter({ hasText: /^SupportersJAJames AndersonMGMonica Geller$/ }));
        await page.waitForTimeout(1500);
        await page.locator('.MuiGrid-root > .MuiButtonBase-root').first().click();
        await page.waitForTimeout(1500);
    ...
    test('Updating membership', async ({ page }) => {
        ...
        await page.locator('input[name="isDonor"]').uncheck();
        await page.getByRole('button', { name: 'Save' }).click();

        //await page.getByRole('button', { name: 'Edit groups' }).click();
        //await expect(page.getByRole('heading', { name: orgName })).toBeVisible();
        //await expect(page.locator('div').filter({ hasText: `^${orgName}Business$`}).locator('span')).toBeVisible();
        //await expect(page.getByText('You have joined this group...')).toBeVisible();
    })
});
